...
class NestedItemSerializer(serializers.Serializer):
	...
	def to_representation(self, instance):
		...
		queryset_purchase = Purchase.objects.filter(item_code=instance.id)
		queryset_sell = Sell.objects.filter(item_code=instance.id)

		if get_start_date and get_end_date:
			queryset_purchase = queryset_purchase.filter(date__range=[get_start_date, get_end_date])
			queryset_sell = queryset_purchase.filter(date__range=[get_start_date, get_end_date])

			first_purchase = queryset_purchase.filter(date__range=[get_start_date, get_end_date]).order_by('id')[:1]
			last_purchase = queryset_purchase.filter(date__range=[get_start_date, get_end_date]).order_by('-id')[:1]
			...
			if len(queryset_purchase) > 1:
				last_purchase = queryset_purchase.distinct()
			...
			for item in queryset_sell:
				...
				if item.out_qty() > first_in_qty:
					...
					item.out_qty = item.out_qty()
					item.out_price = last_in_price
					item.out_total = item.out_qty * item.out_price
					item.stock_qty = item.out_qty
					item.stock_price = item.out_price
					item.stock_total = item.out_qty * item.out_price
